...
coupling=squeeze(mean(delta_activity(14:end,:,:,:))-mean(delta_activity(1:7,:,:,:)));

del = diff(coupling.*(cat(3,ddd,ddd)>30).*(cat(3,ddd,ddd)<200000),[],3);
del = abs(del);
...
for i = 1:n
    ind = find(DDD{1}(i,:)>30 & DDD{1}(i,:)<200);
    day0 = nanmean(delta_activity(:,i,ind,1),3);
    ind = find(DDD{2}(i,:)>30 & DDD{2}(i,:)<200);
    day1 = nanmean(delta_activity(:,i,ind,2),3);
    eday0 = nanstd(delta_activity(:,i,ind,1),0,3)/sqrt(length(ind)*10);;
    day0 = day0 - nanmean(day0(1:7,:));
    day1 = day1 - nanmean(day1(1:7,:));
    day0(stm,:) = [];
    day1(stm,:) = [];
...
clf
Day0 = Day0 - repmat(mean(Day0(1:5,:)),size(Day0,1),1);
Day1 = Day1 - repmat(mean(Day1(1:5,:)),size(Day1,1),1);
tt = tt - tt(stm(end));
plot(tt,medfilt1(Day0(:,cn),1),'k');hold on;
plot(tt,medfilt1(Day1(:,cn),1),'m');
...
[dfo,disto,Fo,epoch,tsta,raw0,df_allo] = BCI_dat_extract2(old,old.bases{ind});

ind =cell2mat(cellfun(@(x) ~isempty(strfind(x,'neuron')),new.bases,'uni',0))
len=cell2mat(cellfun(@(x) length(x),new.siFiles,'uni',0))
ind = find(ind.*len==max(ind.*len))
[dfn,distn,Fn,epoch,tsta,rawn,df_alln] = BCI_dat_extract2(new,new.bases{ind});
%%
clf
dt_si = 1/dat.siHeader.SI.hRoiManager.scanVolumeRate
...
plot(dSel,mean(abs(del),2),'ko','color',[.6 .6 .6]*0,'markerfacecolor','w');hold on;

% mean_bin_plot(dSel,mean(del,2),6,1,1,'k');hold on;
plot(dSel(cn),mean(abs(del(cn,:)),2),'ko','markerfacecolor','r');
plot(xlim,xlim*0,'k:');
figure_finalize
...
vcd = mean(fold(40:100,:))';
% vcd = mean(fold(end-20:end,:))';
lag = 5;
cc = corr(df_all{1}(1+lag:end,:),df_all{1}(1:end-lag,cn));
% cc = corr(df_all{2}(1+lag:end,cn),df_all{2}(1:end-lag,:))';
for i = 1:size(ddd,2);
    vstim = (ddd(:,i)<30);
    stmCD(i) = vstim'*vcd;
    stmCN(i) = vstim'*cc;
    
    vstim = coupling(:,i,2).*(ddd(:,i)>30 & ddd(:,i)<7000);
    coupleCD2(i) = vstim'*vcd;
     vstim = coupling(:,i,1).*(ddd(:,i)>30 & ddd(:,i)<7000);
    coupleCD1(i) = vstim'*vcd;
end
% stmCN = abs(stmCN);
% subplot(131);
...
del = diff(coupling.*(cat(3,ddd,ddd)>30).*(cat(3,ddd,ddd)<200000),[],3);
% del = abs(del);
scatter(stmCD(ind),del(cn,ind),'b');hold on;
mean_bin_plot(stmCD(ind),del(cn,ind),4);
xlabel('Corr(stim group_i,CN)');
ylabel('\Delta W_{i,CN}')
[ccc,p]=corr(stmCD(ind)',del(cn,ind)')
figure_finalize
%%
clf
n = length(dat.roi);
...
% figure(21);clf
clf
del = (mean(Day1(16:20,:))-mean(Day0(16:20,:)))';
plot(dist,mean(del,2),'o','color',[.6 .6 .6]);hold on;
fixed_bin_plots(dist*500/333,mean(del,2),[0 50:100:800],1,'k');hold on;
plot(dist(cn)*500/333,mean(del(cn,:),2),'ko','markerfacecolor','r');
xlim([-40 800]);
