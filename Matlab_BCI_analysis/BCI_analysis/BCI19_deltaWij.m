...
%%
amp = squeeze(mean(Fstim(15:20,:,:))-mean(Fstim(1:8,:,:)));
clear E L se sl
q = ddd*500/333;
for si = 1:max(seq);
    ind = find(seq==si);
    ...
    cls = find(q(:,si)>30 & q(:,si)<80);
    se{si} = nanmean(amp(cls,ind(1:m))');
    sl{si} = nanmean(amp(cls,ind(1+m:end))');    
%     se{si} = nanmean(amp(cls,ind(2:2:end))');
%     sl{si} = nanmean(amp(cls,ind(1:2:end))');    
end
clf
mean_bin_plot([se{:}],[sl{:}],6,1,1,'k')
% plot([se{:}],[sl{:}],'k.')
% hold on;
% plot(xlim,xlim,'k:');
[cc,p]=corr([se{:}]',[sl{:}]')
