...
mi = 8;
struct2ws(dat(si));
Fo = dat(si-1).Fraw;dfo = dat(si-1).df;
n = size(F,2);
F = Fraw;
for ci = 1:n
    a = F(:,ci,:);
...
%             subplot(3,5,8);
%             scatter(mean(fo(tind,:)),mean(f(tind,:)),'ko');
%             xlabel('Amp. Day 0');
%             ylabel('Amp. Day 1');
%
...
len = 10;
plot(conv(nanmean(k(tind,:)),ones(len,1))/len,'k')
box off
xlim([len size(k,2)]);
xlabel('Trial #');
ylabel('Amp.');
...
[u,s,v] = svd(fo1);lv = mean(f1(tind,:))-mean(fo1(tind,:));
for i = 1:size(v,2);
...
off_manifold_frac(mi,si) = (sum(f2(tind,cn)-fo2(tind,cn))-sum(d2(tind,cn)))/sum(f2(tind,cn)-fo2(tind,cn));
off_manifold(mi,si) = (sum(d2(tind,cn)));
delta_cn(mi,si)    = sum(f2(tind,cn)-fo2(tind,cn));
plot(tsta,d2(:,cn),'color',[.7 .7 .7]);hold on;
d2 = (f2-fo2)*v(:,end)*v(:,end)';
%             on_manifold = sum(d2(tind,cn))/sum(f2(tind,cn)-fo2(tind,cn));
